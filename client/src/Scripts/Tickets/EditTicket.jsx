...
import '../../styles/tickets/edit-tickets.css'
const NewTicket = ({setIndexesForDisplay,ticketAuthor,currentProjectName,changeNewTicketState,setleftArrowState,setrightArrowState,currentTicket,userTickets,setUserTickets,setStoreAllTickets
,currentUser}) => {
...
//Add new ticket ot the database
  const onSubmit = ()=>{
      ...
      if((ticketTitle.length===0) || (ticketDescription.length===0)){
      ...
      for(let i = 0;i<userTickets.length;i++){
        if(userTickets[i].ticketTitle === currentTicket){
         
          createdAt = userTickets[i].createdAt
        }
      }
      ...
      const configuration = {
        method:"put",
        url:"http://localhost:4000/tickets/edit-ticket",
        data:{
         currentTicket,
         ticket,
         currentUser
